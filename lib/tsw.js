...
function tswOnLoadMain() {
    ...
    jQuery('body').on('click', '.tsw-btn-select-item', function() {
        // TODO
        console.log('Select item..');
    });

    jQuery('body').on('click', '.tsw-btn-edit-item', function() {
        window.location.href = jQuery('#tsw_path').val() + '?page=the-seo-workspace&id=' + jQuery(this).data('id');
    });
}
        ...
        columnDefs: [
            ...
            { 
                ...
                'render': function ( data, type, row, meta ) {
                    return '<a href="#" data-id="' + data[0] + '" class="button button-green tsw-btn-select-item">Select</a>' + 
                    '<a href="#" data-id="' + data[0] + '" class="button button-green tsw-btn-edit-item">Edit</a>';
                }
            }
        ],
