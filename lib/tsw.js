...
function tswStudySite() {
    ...
    tsw_quantity_per_batch = document.getElementById('quantity_per_batch').value;
    tsw_time_between_batches = document.getElementById('time_between_batches').value;
    tsw_max_secs_allowed = document.getElementById('tsw-current-selected-max_secs_allowed').value;
...
function tswStudySiteSendAjax() {
    ...
    if (tsw_total_secs_executed < tsw_max_secs_allowed) {
        ...
        xhr.send('action=tsw_do_batch&site-id=' + document.getElementById('tsw-current-selected-id').value);
    } else {
        document.getElementById('tsw-box-study-site-status').innerHTML = 'Studying with current status: <strong>finished, max secs achieved</strong>';
        tswStopAll();
    }
}

function tswUpdateStatusBar() {
    if (document.getElementById('tsw-current-selected-id')) {
        let xhr = new XMLHttpRequest();
        ...
        xhr.open('POST', tsw_ajax_url);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
        xhr.send('action=tsw_get_status&site-id=' + document.getElementById('tsw-current-selected-id').value);
    }
}
